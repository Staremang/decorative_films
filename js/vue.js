var d = [
	{
		...
		"content": {
			...
			"description": [
				{
					"text": "Самоклеющаяся виниловая пленка, с декоративным эффектом, в виде вкраплений искрящихся микро-кристаллов.",
					"tooltip": "Структурированная поверхность пленки, в сочетании с вкраплениями из искрящихся микро-кристаллов, хорошо просматриваемых с обеих сторон материала, создает эффект игры света - 'искрящийся блеск' - как на гранях россыпи из множества бриллиантов. <br><br>Пленка - НЕ прозрачна на просвет!<br> Эффектно смотрится с обеих сторон! <br><br>Окрашенной является только лицевая поверхность. Обратная сторона имеет серебристо-серый оттенок. <br><br>Предназначена для использования только на ровных поверхностях, как прозрачных, так и не непрозрачных."
				}, {
					"text": "Пленка предназначена для дизайнерского оформления интерьеров, сплошного либо частичного нанесения, на ровные поверхности.",
					"tooltip": "Материал применяется при оформление офисных и функциональных перегородок; для дизайна шкафов- купе; при декорировании окон и витражей; в душевых и ванных комнатах, и других целей."
				}
			],
			"properties": [
				{
					"text": "Пленка - НЕ прозрачна на просвет. <br>Цветной является только лицевая сторона. <br>Обратная сторона - серебристо-серого цвета.",
					"iconClass": "icon-lens"
				}, {
					"text": "Применима для плоттерной резки. <br>Возможна экосольвентная печать!",
					"iconClass": "icon-plotter-2",
					"tooltip": "Пленку можно использовать для плоттерной резки векторных изображений. Из-за особенностей структуры материала, несколько нежелательно наличие тонких элементов, менее 3мм. <br>Иных ограничений не имеется! <br><br>Возможна экосольвентная цифровая печать."
				}
			],
			"proportions": {
				"tooltip": "Пленка поставляется с полиакрилатным клеевым слоем на основе растворителя, постоянной фиксации. <br><br>Подложка - Силиконизированная бумага (120g) <br><br>Срок эксплуатации: свыше 3 лет (внутри помещений)"
			}
		},
		"sub": [
			{
...
Vue.component('film-description', {
	...
	template: `
			<div v-if="selectedFilm">
				<h2 class="films-description__title">{{selectedFilm.parentName}}</h2>
				<h3 class="films-description__subtitle">{{selectedFilm.name}}</h3>
				<div class="films-description__content">
					<div>
						<div class="tooltip">
							...
							<div class="tooltip__content">
								<table>
									<tbody>
										...
										<tr>
											<td>от 1 до 10 метров</td>
											<td>{{ selectedFilm.content.price[1] }} руб./м.п.</td>
										</tr>
										<tr>
											<td>свыше 10 метров</td>
											<td>{{ selectedFilm.content.price[2] }} руб./м.п.</td>
										</tr>
										<tr>
											<td>кратно рулону</td>
											<td>{{ selectedFilm.content.price[3] }} руб./м.п.</td>
										</tr>
									</tbody>
								</table>
							</div>
						</div>
						<div class="films-description__price">от {{ selectedFilm.content.price[0] }}** RUB за 1м<sup>2</sup></div>
						<div class="films-description__price-note">** стоимость указана при приобретении рулона</div>
					</div>
					
					<div 
					class="films-description__text"
					v-for="description in selectedFilm.content.description">
						<div 
						v-if="description.tooltip" 
						class="tooltip">
							<div class="tooltip__icon">*</div>
							<div class="tooltip__content" v-html="description.tooltip"></div>
						</div>
						<span v-html="description.text"></span>
					</div>
					
					<div class="films-description__prop-title">Характеристики / Свойства:</div>
					<ul class="films-description__properties">
						<li
						v-for="properties in selectedFilm.content.properties"
						:class="['films-description__properties-item', properties.iconClass]">
							<div class="tooltip"
							v-if="properties.tooltip">
								<div class="tooltip__icon">*</div>
								<div class="tooltip__content" v-html="properties.tooltip"></div>
							</div>
					...
					<div>
						<div class="tooltip">
							<div class="tooltip__icon">*</div>
							<div class="tooltip__content" v-html="selectedFilm.content.proportions.tooltip"></div>
						</div>
						<ul class="properties-border">
				...
				<div class="films-gallery">
					<a 
					class="films-gallery__item" 
					data-fancybox="gallery" 
					v-for="img in selectedFilm.galleryPrev"
					:href="img[0]">
						<img :src="img[1]">
					</a>
				</div>
...
Vue.component('list', {
	...
	template: `
		<ul>
			<li
				v-for="t in model"
				:class="['films-menu__title', {active: openKey === t.category}]">
				<span 
					class="films-menu__title-link"
					@click="openKey = t.category">
				{{t.name}}
				</span>
				<ul class="films-menu__subtitle-block">
					<li 
					v-for="f in t.sub"
					:class="['films-menu__subtitle', {active: 0}]" 
					@click="">
					{{f.name}}
					</li>
				</ul>
...
var app = new Vue({
	...
	methods: {
		changeFilm: function (id) {
			...
			this.type.some(function (item, i) {
				parent = item;
